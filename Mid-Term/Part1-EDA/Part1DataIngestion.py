...
df16 = pd.read_csv('../data/properties_2016.csv', low_memory=False)
df17 = pd.read_csv('../data/properties_2017.csv', low_memory=False)

# Then import the transaction data
df_transactions16 = pd.read_csv('../data/train_2016_v2.csv', low_memory=False)
df_transactions17 = pd.read_csv('../data/train_2017.csv', low_memory=False)
...
df_total.drop('regionidcity',axis=1, inplace=True)
df_total.drop('regionidcounty',axis=1, inplace=True)
df_total.drop('propertyzoningdesc',axis=1, inplace=True)
df_total.drop('taxdelinquencyyear',axis=1, inplace=True)
df_total.drop('propertycountylandusecode',axis=1, inplace=True)
...
### Convert Booleans and Categorical Variables to One-Hots
...
df_total['hashottuborspa'] = df_total['hashottuborspa'].map(lambda a: hottub(a))
...
df_total['fireplaceflag'] = df_total['fireplaceflag'].map(lambda a: fire(a))
...
df_total['taxdelinquencyflag'] = df_total['taxdelinquencyflag'].map(lambda a: tax(a))

fips = pd.get_dummies(df_total['fips'])
df_total = df_total.join(fips)

yr = pd.get_dummies(df_total['setyear'])
df_total = df_total.join(yr)

ac = pd.get_dummies(df_total['airconditioningtypeid'], prefix='ac')
df_total = df_total.join(ac)

heat = pd.get_dummies(df_total['heatingorsystemtypeid'], prefix='heat')
df_total=df_total.join(heat)

pool = pd.get_dummies(df_total['pooltype'], prefix='pooltype')
df_total = df_total.join(pool)

### Upload Output to AWS
output="clan_data.csv"
