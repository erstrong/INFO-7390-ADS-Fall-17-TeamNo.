...
#print(div)
	...
	def output(self):
		return luigi.LocalTarget("rawloanstats.csv")


## Data Cleansing
class cleanMissingData(luigi.Task):
	def requires(self):
		yield scrapeLoanData()
	
	def run(self):
		loans=pd.read_csv(scrapeLoanData().output().path, encoding = "ISO-8859-1")
...
class AggregateData(luigi.Task):
	def requires(self):
		yield cleanMissingData()
		#yield scrapeDeclineData()
	
	def run(self):
		loans=pd.read_csv(cleanMissingData().output().path, encoding = "ISO-8859-1")
		...
		loanData=loans[columns]
		loanData.to_csv(self.output().path,index=False)

	def output(self):
		return luigi.LocalTarget('classificationLoanstatsData.csv')
